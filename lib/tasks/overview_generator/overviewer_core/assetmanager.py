...
directory. 
        ...
        try:
            with open(os.path.join(self.js_outputdir, "overviewerConfig.js")) as c:
                overviewerConfig_str = "{" + "\n".join(c.readlines()[1:-1]) + "}"
            self.overviewerConfig = json.loads(overviewerConfig_str)
        except Exception, e:
            if os.path.exists(os.path.join(self.js_outputdir, "overviewerConfig.js")):
    ...
    def _output_assets(self, tilesets, initial):
        ...
        with FileReplacer(os.path.join(self.js_outputdir, "overviewerConfig.js")) as tmpfile:
